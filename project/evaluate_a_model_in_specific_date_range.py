...
import numpy as np
domain = np.arange(0,amountOfDaysOfRange*12-1)
axs.plot(domain, in_seq2[12:-1],linewidth = '5')
axs.plot(domain, predictedValues[1:], linewidth = '5')
...
print("\n OPTION 2 errors for "+techniquedUsedForTitle+": ")
mse = mean_squared_error(in_seq2[12:-1],predictedValues[1:])
rmse = sqrt(mse)
print("MSE  = ", mse)
print("RMSE  = ", rmse)
...
if(not isTestSet):
    ...
    axs[0].plot(domain, in_seq2[12:-1],linewidth = '5')
    axs[0].plot(domain, predictedValues[1:], linewidth = '5')
    ...
    axs[1].plot(domain, dfAux["SYM-H"][:-1], color="black", linewidth = '5')
